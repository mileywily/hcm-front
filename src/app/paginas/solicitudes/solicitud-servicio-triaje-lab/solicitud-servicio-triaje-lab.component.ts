...
@Component({
  selector: 'app-solicitud-servicio-triaje-lab',
  templateUrl: './solicitud-servicio-triaje-lab.component.html',
  styleUrls: ['./solicitud-servicio-triaje-lab.component.scss'],
  providers: [MessageService,ConfirmationService]
})
export class SolicitudServicioTriajeLabComponent implements OnInit {
...
handleChange(e, registro: any) {
  ...
  fullDropdownTipoOrden() {
    this.tipoOrdenService.getAllByTipoSolicitud(this.menuService.getTipoSolicitud())
      ...
      .then((results) => {
        ...
        let aux=[];
        aux=results;
        this.tipoOrdenes = aux.filter(x => x.tipo_atencion_id == this.tipoAtencionId);
    ...
    if (this.solicitudId != -1){
     ...
     .then(results => {
       if (results){
         ...
         this.consultarExamenesRequeridos();
  ...
  crearOrden() {
    //El selected en este caso corresponde al elemento seleccionado en la tabla de la vista
      this.navigate('orden-servicio-triaje-lab', this.solicitud.solicitud_id, this.tipoSolicitud.tipo_solicitud_id, this.solicitud.especialidad_id);
  }
  ...
  addExamenes() {
    if (this.examenesSolicitadosSel.length > 0) {
      //this.examenesAgregados = [];
      for (let i = 0; i < this.examenesSolicitadosSel.length; i++)
      {
          let aux=[];
          aux = this.examenesAgregados.filter(x => x.examen_id == this.examenesSolicitadosSel[i].examen_id);
          if(aux.length <= 0){
            let data = {
              examen_id: this.examenesSolicitadosSel[i].examen_id,
              nombre_grupo: this.examenesSolicitadosSel[i].nombre_grupo,
              nombre: this.examenesSolicitadosSel[i].name,
              asignado: 0,
            };
            
            this.pushExamen(data);
          }
      }
  ...
  addExamenesPrevio() {
    if (this.reporteExamenes.length > 0) {
      this.examenesAgregados = [];
      for (let i = 0; i < this.reporteExamenes.length; i++)
      {
         // let aux=[];
         // aux = this.examenesAgregados.filter(x => x.examen_id == this.examenesSolicitadosSel[i].examen_id);
         // if(aux.length <= 0){
            let data = {
              examen_id: this.reporteExamenes[i].examen_id,
              nombre: this.reporteExamenes[i].nombre_examen,
              
              nombre_grupo: this.reporteExamenes[i].nombre_grupo,
              asignado: this.reporteExamenes[i].asignado
            };
