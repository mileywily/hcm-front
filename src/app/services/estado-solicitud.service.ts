...
export class EstadoSolicitudService {
  ...
  constructor(private http: HttpClient) {
    this.url = environment.apiUrl + 'estadosolicitudes';
  }

  getAll(): Observable<EstadoSolicitud[]> {

    let apiURL = this.url; 
    return this.http.get<EstadoSolicitud[]>(apiURL);
  }

  getAllCombo(): Observable<EstadoSolicitud[]> {
  ...
  getByIdRequestType(id: number): Observable<any> {
    ...
    let apiURL = this.url+'/estadosolicitudes/'+id; 
    return this.http.get<any>(apiURL);
  }

  create(registro: EstadoSolicitud) {
  ...
  update(registro: EstadoSolicitud) {
    const url = `${this.url}`;
  ...
  deleteByLote(registros: EstadoSolicitud[]) {
  ...
  private log(message: string) {
    console.log('EstadoSolicitudService: ' + message);
  }
