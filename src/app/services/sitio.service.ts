...
export class SitioService {
  ...
  constructor(private http: HttpClient) {
    this.url = environment.apiUrl + 'sitios';
  }

  getAll(): Observable<Sitio[]> {

    let apiURL = this.url; 
    return this.http.get<Sitio[]>(apiURL);
  }

  getAllCombo(): Observable<Sitio[]> {
  ...
  getByIdRequestType(id: number): Observable<any> {
    ...
    let apiURL = this.url+'/sitios/'+id; 
    return this.http.get<any>(apiURL);
  }

  create(registro: Sitio) {
  ...
  private log(message: string) {
    console.log('SitioService: ' + message);
  }
