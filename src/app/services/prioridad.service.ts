...
export class PrioridadService {
  ...
  constructor(private http: HttpClient) {
    this.url = environment.apiUrl + 'prioridades';
  }

  getAll(): Observable<Prioridad[]> {

    let apiURL = this.url; 
    return this.http.get<Prioridad[]>(apiURL);
  }

  getAllCombo(): Observable<Prioridad[]> {
  ...
  getByIdRequestType(id: number): Observable<any> {
    ...
    let apiURL = this.url+'/prioridades/'+id; 
    return this.http.get<any>(apiURL);
  }

  create(registro: Prioridad) {
  ...
  deleteByLote(registros: Prioridad[]) {
  ...
  private log(message: string) {
    console.log('PrioridadService: ' + message);
  }
