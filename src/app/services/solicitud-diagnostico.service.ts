...
@Injectable({
  ...
  export class SolicitudDiagnosticoService 
  {

    private url: string;

    constructor(private http: HttpClient) {
      this.url = environment.apiUrl + 'solicitudiagnosticos';
    }
    ...
    getDiagnosticosBySolicitud(solicitud_id): Observable<any[]> {
      let apiURL = this.url + '/grupoexamen/solicitud/'+solicitud_id;
      return this.http.get<any[]>(apiURL);
    }
  
    getDiagnosticosAsignadosBySolicitud(solicitud_id): Observable<any[]> {
