...
@Component({
  selector: 'app-orden-modificar-triaje',
  templateUrl: './orden-modificar-triaje.component.html',
  styleUrls: ['./orden-modificar-triaje.component.scss'],
  providers: [MessageService,ConfirmationService]
})
export class OrdenModificarTriajeComponent implements OnInit 
{
  ...
  roles: RolModelo[] = [];
  /* Codigo de los roles */
  codRolImagenModificarOrden: string;
  codRolImagenCancelarOrden: string;

  activarBotonModificarOrdenImagen: boolean = false;
  activarBotonCancelarOrdenImagen: boolean = false;
  ...
  ) 
  {
    ...
    this.cols = [
      ...
      { field: 'flag', header: 'Grupo', width: '2%' },
  ...
  ngOnInit(): void 
  {
    ...
    this.codRolImagenModificarOrden   =  this.rolesService.getCodRolImagenModificarOrden();
    this.codRolImagenCancelarOrden = this.rolesService.getCodRolImagenCancelarOrden();
       
    this.activarBotonModificarOrdenImagen = this.srvRolesByUser.buscarRolPorCodigo(this.codRolImagenModificarOrden).length > 0 ? true : false;
    this.activarBotonCancelarOrdenImagen = this.srvRolesByUser.buscarRolPorCodigo(this.codRolImagenCancelarOrden).length > 0 ? true : false;
...
deleteSelectedItems() {
 ...
 setear_parametros()
 {
  ...
  let format = 'yyyy-MM-dd';
         ...
         if(!this.fecha_inicio || this.fecha_inicio == null || this.fecha_inicio ==undefined ||
           ...
           else
           {
            ...
            if (  this.serviciosSolicitadosSel !=undefined)
            {
  ...
  consultar()
  {
     ...
     this.detalleSolicitudService.Rep_orden_modificar_triaje(this.parametros)
      .toPromise()
      .then(results => 
        {
            this.Atenciones = results;
