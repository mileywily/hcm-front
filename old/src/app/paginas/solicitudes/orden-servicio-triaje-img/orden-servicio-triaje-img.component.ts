...
@Component({
  selector: 'app-orden-servicio-triaje-img',
  templateUrl: './orden-servicio-triaje-img.component.html',
  styleUrls: ['./orden-servicio-triaje-img.component.scss'],
  providers: [MessageService]
})
export class OrdenServicioTriajeImgComponent implements OnInit, OnDestroy {
 ...
 dropdownServiciosPorGrupo(){
  ...
  disponibilidad(){
    ...
    this.router.navigate(['calendario-triaje-img', 'A'] , { relativeTo: this.activatedRoute });
  }
      ...
      if (this.orden.solicitud_id){
        ...
        if (!this.orden.atencion_id) {
          ...
          .then((results) => {
                ...
                "/solicitud-servicio-triaje-img", this.solicitud.beneficiario_id,  this.solicitud.solicitud_id]);
                this.blocked = false;
          })
          .catch((err) => {
            console.log(err);
